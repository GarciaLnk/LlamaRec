...
from inference import load_dataset_map, load_retriever, retrieve_candidates
...
@st.cache_resource
def load_resources() -> (
    tuple[FileIndex, dict[int, str], dict[str, list[tuple[int, str]]], ScriptModule]
):
    ix = load_index()
    dataset_map = load_dataset_map()
    playlist_map = load_playlist_map(dataset_map)
    retriever = load_retriever()
    return ix, dataset_map, playlist_map, retriever
