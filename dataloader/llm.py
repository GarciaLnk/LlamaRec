...
def seq_to_token_ids(
    ...
    data_point = {}
    data_point["system"] = (
        args.llm_system_template
        if args.llm_system_template is not None
...
class LLMDataloader:
    def __init__(self, args, dataset):
        ...
        retrieved_file = pickle.load(
            open(os.path.join(args.llm_retrieved_path, "retrieved.pkl"), "rb")
        )
...
class LLMTrainDataset(data_utils.Dataset):
    ...
    def __getitem__(self, index):
        ...
        samples = self.rng.randint(
            1, self.args.num_items + 1, size=5 * self.args.llm_negative_sample_size
        )
