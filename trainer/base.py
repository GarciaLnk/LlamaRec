...
class BaseTrainer(metaclass=ABCMeta):
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.args.val_strategy == "epoch":
                self.exit_training = self.validate(
                    epoch, accum_iter
                )  # val after every epoch
    ...
    def train_one_epoch(self, epoch, accum_iter):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            accum_iter += 1
            if (
                self.args.val_strategy == "iteration"
                and accum_iter % self.args.val_iterations == 0
            ):
                self.exit_training = self.validate(
                    epoch, accum_iter
                )  # val after certain iterations
                if self.exit_training:
                    break
    ...
    def _create_loggers(self):
        ...
        val_loggers.append(
            BestModelLogger(self.args, model_checkpoint, metric_key=self.best_metric)
        )
